...
namespace ApiEmpleadosMultiplesRoutes.Controllers
{
    ...
    public class EmpleadosController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("[action]/{oficio}")]
        public async Task<List<Empleado>> EmpleadosOficio(string oficio)
        {
            return await this.repo.GetEmpleadosOficioAsync(oficio);
        }

        //LOS PARAMETROS DEBEN TENER EL MISMO NOMBRE
        //DEBEN ESTAR EN EL MISMO ORDEN QUE REICBE EL METODO
        [HttpGet]
        [Route("[action]/{salario}/{iddepartamento}")]
        public async Task<ActionResult<List<Empleado>>> EmpleadosSlario(int salario, int iddepartamento)
        {
